...
+ (NSArray *)_dayNames {
	...
	_dayNames = [[formatter weekdaySymbols] copy];
	[formatter release];
...
- (void)_drawTitleRect:(NSRect)titleRect {
	...
	[[self _textColor] set];
	AKDrawStringAlignedInFrame([currentMonth descriptionWithCalendarFormat:@"%B %Y" timeZone:nil locale:nil], [NSFont fontWithName:@"Helvetica Bold" size:1], NSCenterTextAlignment, NSIntegralRect(monthTitleRect));
	...
	for (NSUInteger currentDay = 0; currentDay < [dayNames count]; currentDay++) {
		...
		NSString *string = [dayNames objectAtIndex:currentDay];
